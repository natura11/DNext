@VFALFT-22390 @VFALFT-22389 @fiber @regression @activation
Feature: Fiber Activation
  ...
  Background:
    ...
    And   User fills in enter Customer name field with valid name as "SCHUPPE AND SONS AUTOMATION" on Customer360 search page
...
#    When  User selects the "Digitalb Premium Plus" product on DSales Product page
    ...
    And   User selects the "FIBER" as infrastructure on Product Detail page
    ...
    And User should see the name of "SCHUPPE AND SONS AUTOMATION" progressed customer on BackOffice page
    And User clicks the name of "SCHUPPE AND SONS AUTOMATION" customer on BackOffice page
